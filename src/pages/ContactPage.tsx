...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.email.trim() || !formData.subject.trim() || !formData.message.trim()) {
      toast.error(locale === 'de' ? 'Bitte füllen Sie alle Felder aus' : 
                  locale === 'cz' ? 'Vyplňte prosím všechna pole' : 
    ...
    if (!emailRegex.test(formData.email)) {
      toast.error(locale === 'de' ? 'Ungültige E-Mail-Adresse' : 
                  locale === 'cz' ? 'Neplatná e-mailová adresa' : 
                ...
                {isSuccess ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <h3 className="text-xl font-semibold mb-2">
                      {locale === 'de' ? 'Nachricht gesendet!' : 
                       locale === 'cz' ? 'Zpráva odeslána!' : 
                       'Message sent!'}
                    </h3>
                    <p className="text-muted-foreground mb-6">
                      {locale === 'de' ? 'Wir werden uns so schnell wie möglich bei Ihnen melden.' : 
                       locale === 'cz' ? 'Ozveme se vám co nejdříve.' : 
                       'We will get back to you as soon as possible.'}
                    </p>
                    <Button onClick={() => setIsSuccess(false)}>
                      {locale === 'de' ? 'Weitere Nachricht senden' : 
                       locale === 'cz' ? 'Odeslat další zprávu' : 
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          placeholder={locale === 'de' ? 'Ihr Name' : locale === 'cz' ? 'Vaše jméno' : 'Your name'}
                          maxLength={100}
                          required
                      ...
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          placeholder={locale === 'de' ? 'ihre@email.de' : locale === 'cz' ? 'vas@email.cz' : 'your@email.com'}
                          maxLength={255}
                          required
                          disabled={isSubmitting}
                        />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input 
                        ...
                        placeholder={locale === 'de' ? 'Betreff' : locale === 'cz' ? 'Předmět' : 'Subject'}
                        maxLength={200}
                        required
                        disabled={isSubmitting}
                      />
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea 
                        ...
                        placeholder={locale === 'de' ? 'Ihre Nachricht...' : locale === 'cz' ? 'Vaše zpráva...' : 'Your message...'}
                        rows={6}
                        maxLength={5000}
                        required
                        disabled={isSubmitting}
                    ...
                    <Button type="submit" className="w-full" disabled={isSubmitting}>
                      {isSubmitting ? (
                        <>
                          ...
                          {locale === 'de' ? 'Wird gesendet...' : locale === 'cz' ? 'Odesílání...' : 'Sending...'}
                        </>
                      ) : (
                        t.contact.send
                      )}
