...
export function HighlightsSection({ classIds = [], compact = false }: { classIds?: string[]; compact?: boolean }) {
  ...
  return <section aria-label={c.section}><div className={`flex items-center justify-between ${compact ? 'mb-2' : 'mb-3'}`}><h2 className={`flex items-center gap-2 font-bold ${compact ? 'text-lg sm:text-2xl' : 'text-xl sm:text-2xl'}`}><Sparkles className="h-5 w-5 text-primary"/>{c.section}</h2><span className="text-xs text-muted-foreground">{index + 1}/{candidates.length}</span></div><article className="relative overflow-hidden rounded-xl bg-slate-950 text-white"><img src={candidate.vehicleImageUrl!} alt={`${candidate.driverName}, ${[candidate.vehicleMake, candidate.vehicleModel].filter(Boolean).join(' ')}`} className={`w-full object-cover ${compact ? 'aspect-[16/9] sm:aspect-[16/7]' : 'aspect-[4/3] sm:aspect-[16/8]'}`} loading="eager" decoding="async"/><div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/10 to-transparent"/><div className={`absolute inset-x-0 bottom-0 ${compact ? 'p-3 sm:p-6' : 'p-4 sm:p-6'}`}><p className="text-xs font-bold uppercase tracking-wider text-white/70">{candidate.startNumberNorm ? `${c.start} ${candidate.startNumberNorm}` : c.participant}</p><h3 className={`font-black ${compact ? 'mt-0.5 text-lg sm:mt-1 sm:text-3xl' : 'mt-1 text-2xl sm:text-3xl'}`}>{candidate.driverName}</h3><p className={`text-white/75 ${compact ? 'text-xs sm:text-sm' : 'text-sm'}`}>{[candidate.vehicleMake, candidate.vehicleModel, candidate.vehicleYear].filter(Boolean).join(' · ')}</p></div><Button size="icon" variant="secondary" aria-label={c.expand} className="absolute right-3 top-3" onClick={() => setExpanded(true)}><Expand/></Button>{candidates.length > 1 && <><Button size="icon" variant="secondary" aria-label={c.previous} className="absolute left-3 top-1/2 -translate-y-1/2" onClick={() => move(-1)}><ChevronLeft/></Button><Button size="icon" variant="secondary" aria-label={c.next} className="absolute right-3 top-1/2 -translate-y-1/2" onClick={() => move(1)}><ChevronRight/></Button></>}</article><Dialog open={expanded} onOpenChange={setExpanded}><DialogContent className="max-w-5xl border-0 bg-black p-2"><img src={candidate.vehicleImageUrl!} alt={candidate.driverName} className="max-h-[88vh] w-full object-contain"/></DialogContent></Dialog></section>;
}
